...
public class Bearer implements MiniGame_H {
    ...
    public void go(int mode, int lv_) throws IOException {
        this.lv = (byte) lv_;
    ...
    private void draw0(int x, int y, int h) {
        ...
        Ui uiI = Ui.i();
        int i = (Player.REALIZED + 10) * 2;
        uiI.drawK(x + 420 + 10, y + 25 + 5, Player.REALIZED - 10, h2, 4);
        ...
        uiI2.drawString(Constants_H.GAME_TXT_14, x + 420 + 20, ty, 0, 0, 0);
        int ty2 = ty + 50;
        for (int i3 = 0; i3 < this.gDate[0].length; i3 += 2) {
            GameRun gameRun = this.gr;
            short s = this.gDate[0][i3];
            int i4 = (Player.REALIZED + 10) * 2;
            gameRun.drawItem(s, x + 420 + 25 + (i3 % 4 == 2 ? 100 : 0), ((i3 >> 2) * 30) + ty2, 0);
            Ui uiI3 = Ui.i();
            String str = String.valueOf(this.gDate[0][i3 + 1] < 0 ? "" : "+") + ((int) this.gDate[0][i3 + 1]);
            int i5 = (Player.REALIZED + 10) * 2;
            uiI3.drawString(str, x + 420 + 25 + (i3 % 4 == 2 ? 100 : 0) + 16, (((i3 >> 2) * 30) + ty2) - 4, 0, 0, 1);
        }
        int ty3 = ty2 - 50;
        Ui.i().drawListKY(2, x + Player.REALIZED + 12 + 25, ty3, Player.REALIZED - 50, 3, 30, 10, this.sel, 4, 2);
        int i6 = 0;
        while (i6 < this.gr.about_b.length) {
            System.out.println("i   " + i6);
            Ui.i().drawString(this.gr.about_b[i6].toString(), x + Player.REALIZED + 15 + 25 + 26, ((i6 * 40) + ty3) - 2, 0, this.sel == i6 ? 0 : 3, 1);
            if (this.gr.pkey.isSelect(x + Player.REALIZED + 12 + 25, ((i6 * 40) + ty3) - 2, Player.REALIZED - 50, 40)) {
