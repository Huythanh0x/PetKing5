...
public class Racing implements MiniGame_H {
    ...
    private void draw0(int x, int y, int h, int fh) {
        ...
        uiI.drawK(x + 420, y + fh + 3, Player.REALIZED, h, 4);
        Ui uiI2 = Ui.i();
        StringBuffer[] stringBufferArr = this.gr.about_a;
        int i3 = (Player.REALIZED + 10) * 2;
        uiI2.drawStringY(stringBufferArr, x + 420 + 25, y + fh + 15, fh, 0, 0);
        if (this.state == 0) {
            Ui.i().drawString(Constants_H.GAME_TXT_4, x + Player.REALIZED + 10, y + fh + 15, 0, 0, 0);
            for (int i4 = 0; i4 < this.length; i4++) {
                Ui.i().drawK(x + Player.REALIZED + 10 + 25 + 14, y + fh + 50 + ((fh + 16) * i4), 150, fh + 4, 1);
                Ui.i().drawString(this.gr.about_b[i4].toString(), x + Player.REALIZED + 10 + 25 + 14, y + fh + 50 + ((fh + 16) * i4), 0, 3, 0);
                if (this.gr.pkey.isSelect(x + Player.REALIZED + 10 + 25 + 14, y + fh + 50 + ((fh + 16) * i4), 150, fh + 4)) {
                    if (this.sel == i4) {
    ...
    private void draw1(int x, int y, int h) {
        ...
        while (i < this.length) {
            ...
            int i2 = x + 25 + (i * Key_H.KEY_NUM4) + 22;
            int i3 = y + 25 + 8 + h;
            ...
            uiI.drawActionOne(sprite, 0, i2, i3, bArr, 0, z);
        ...
        int h3 = h2 + 25 + 3;
        for (byte i4 = 0; i4 < this.length; i4 = (byte) (i4 + 1)) {
            Ui.i().drawK(x + 25 + (i4 * Key_H.KEY_NUM4), y + h3, 44, 34, 1);
            this.gr.drawItem(12, x + 25 + (i4 * Key_H.KEY_NUM4), y + h3 + 10, 0);
            Ui.i().drawString("x" + ((int) i4), x + 25 + (i4 * Key_H.KEY_NUM4) + 16, y + h3 + 8, 0, 3, 1);
        }
        if (this.state == 2) {
